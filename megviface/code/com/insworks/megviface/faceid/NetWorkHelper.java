...
public class NetWorkHelper {
     ...
     * 判断手机网络是否通畅
     */
    public static boolean isNetWorkCooection(Context context) {
        int network_type = checkNetWorkConnection(context);

        // 如果手机是飞行模式或者是断开网络状态（停止请求返回）
//		if (network_type == NETWORK_AIRPLANE
//				|| network_type == NETWORK_DISCONNETED) {
        if (network_type == NETWORK_DISCONNETED) {
            return false;
        }
        return true;
    }
    ...
    public static int isDataConnection(Context context) {
        ...
        // AliLog.i("CommonHelper", "isDataConnection >> 飞行模式 = "+Mode_airpln);
        if (Mode_airpln != null && Mode_airpln.equalsIgnoreCase("1")) {
            // AliLog.i("CommonHelper", "isDataConnection >> 飞行模式：开");
            return -1;
        }

        if (networkInfo != null && networkInfo.isConnected()) {
            // AliLog.i("CommonHelper", "isDataConnection >> 网络状态：已连接");
            return 1;
        } else {
            // AliLog.i("CommonHelper", "isDataConnection >> 网络状态：未连接");
            return 0;
        }
    }
